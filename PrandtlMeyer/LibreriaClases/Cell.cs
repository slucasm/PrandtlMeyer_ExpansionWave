...
namespace LibreriaClases
{
    public class Cell
    {
        ...
        public void calculateTransform(double H,double E,double Theta) //Checked
        {
            ...
            {
                dETA_x = (Math.Tan(Theta) / h) - (y_t * (Math.Tan(Theta) / h));
            }
            y = (y_t * h) + y_s;
            dETA_y = 1 / h;
        }
        ...
        public double[] predictorStepBody(double F1_up,double F2_up,double F3_up,double F4_up,double G1_up,double G2_up,double G3_up,double G4_up,double F1_down,double F2_down,double F3_down,double F4_down,double P_up,double P_down,double delta_x,double delta_y_t)
        {
            ...
            double SF1 = ((0.6 * (Math.Abs((P_up) - (2 * P) + P_down))) / (P_up + 2 * P + P_down)) * (F1_up - (2 * F1) + F1_down);
        ...
        {
            A_p = ((Math.Pow(F3_p_next,2)) / (2 * F1_p_next)) - F4_p_next;
            B_p = (Gamma / (Gamma - 1)) * F1_p_next * F2_p_next;
            C_p = -(((Gamma + 1) / (2 * (Gamma - 1))) * (Math.Pow(F1_p_next,3)));
            Rho_p = (-B_p + (Math.Sqrt((Math.Pow(B_p,2)) - (4 * A_p * C_p)))) / (2 * A_p);
            P_p = F2_p_next - ((Math.Pow(F1_p_next,2)) / Rho_p);
            double G1_p_next = Rho_p * (F3_p_next / F1_p_next);
            double G2_p_next = F3_p_next;
            double G3_p_next = (Rho_p * (Math.Pow((F3_p_next / F1_p_next) , 2))) + F2_p_next - ((Math.Pow(F1_p_next , 2)) / Rho_p);
            double G4_p_next = ((Gamma / (Gamma - 1)) * ((F2_p_next) - ((Math.Pow(F1_p_next, 2)) / Rho_p)) * (F3_p_next / F1_p_next)) + (((Rho_p * F3_p_next) / (2 * F1_p_next)) * ((Math.Pow((F1_p_next / Rho_p) , 2)) + (Math.Pow((F3_p_next / F1_p_next) , 2))));
            double[] arrayG_p = { G1_p_next, G2_p_next, G3_p_next, G4_p_next };
            return arrayG_p;
        }


        public double[] calculateCorrectorBody(double F1_p_next, double F1_p_next_up, double F1_p_next_down, double F2_p_next, double F2_p_next_up, double F2_p_next_down, double F3_p_next, double F3_p_next_up, double F3_p_next_down, double F4_p_next, double F4_p_next_up, double F4_p_next_down, double G1_p_next, double G1_p_next_down, double G2_p_next, double G2_p_next_down, double G3_p_next, double G3_p_next_down, double G4_p_next, double G4_p_next_down,double P_p_up,double P_p_down,double delta_x,double delta_y_t)
        {
            dF1_p_x = (dETA_x * ((F1_p_next_down - F1_p_next) / delta_y_t)) + (dETA_y * ((G1_p_next_down - G1_p_next) / delta_y_t));
            dF2_p_x = (dETA_x * ((F2_p_next_down - F2_p_next) / delta_y_t)) + (dETA_y * ((G2_p_next_down - G2_p_next) / delta_y_t));
            dF3_p_x = (dETA_x * ((F3_p_next_down - F3_p_next) / delta_y_t)) + (dETA_y * ((G3_p_next_down - G3_p_next) / delta_y_t));
            dF4_p_x = (dETA_x * ((F4_p_next_down - F4_p_next) / delta_y_t)) + (dETA_y * ((G4_p_next_down - G4_p_next) / delta_y_t));

            double SF1_p = (((0.6 * (Math.Abs(P_p_up) - (2 * P_p) + P_p_down))) / (P_p_up + 2 * P_p + P_p_down)) * (F1_p_next_up - (2 * F1_p_next) + F1_p_next_down);
            double SF2_p = (((0.6 * (Math.Abs(P_p_up) - (2 * P_p) + P_p_down))) / (P_p_up + 2 * P_p + P_p_down)) * (F2_p_next_up - (2 * F2_p_next) + F2_p_next_down);
            double SF3_p = (((0.6 * (Math.Abs(P_p_up) - (2 * P_p) + P_p_down))) / (P_p_up + 2 * P_p + P_p_down)) * (F3_p_next_up - (2 * F3_p_next) + F3_p_next_down);
            double SF4_p = (((0.6 * (Math.Abs(P_p_up) - (2 * P_p) + P_p_down))) / (P_p_up + 2 * P_p + P_p_down)) * (F4_p_next_up - (2 * F4_p_next) + F4_p_next_down);
            ...
            double F1_next = F1 + (dF1_x_av * delta_x) + SF1_p;
            double F2_next = F2 + (dF2_x_av * delta_x) + SF2_p;
            double F3_next = F3 + (dF3_x_av * delta_x) + SF3_p;
            double F4_next = F4 + (dF4_x_av * delta_x) + SF4_p;
        ...
        {
            dF1_p_x = (dETA_x * ((F1_p_next - F1_p_next_up) / delta_y_t)) + (dETA_y * ((G1_p_next - G1_p_next_up) / delta_y_t));
            dF2_p_x = (dETA_x * ((F2_p_next - F2_p_next_up) / delta_y_t)) + (dETA_y * ((G2_p_next - G2_p_next_up) / delta_y_t));
            dF3_p_x = (dETA_x * ((F3_p_next - F3_p_next_up) / delta_y_t)) + (dETA_y * ((G3_p_next - G3_p_next_up) / delta_y_t));
            dF4_p_x = (dETA_x * ((F4_p_next - F4_p_next_up) / delta_y_t)) + (dETA_y * ((G4_p_next - G4_p_next_up) / delta_y_t));
